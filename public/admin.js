...
(function () {
  ...
  function loadAll() {
    setView('dashboard');
    /* Inquiries moved to the ERP (/erp/leads) — this panel is the blog editor
       now. The inquiries plumbing below is left in place but unused so the
       export/import backup format stays readable for old archives. */
    Promise.all([DB.getPosts()])
      .then(function (r) {
        posts     = r[0] || [];
        inquiries = [];
        updateBadges();
  ...
  function renderDashboard() {
    ...
    var weekPosts = posts.filter(function (p) { return new Date(p.updated_at || p.created_at).getTime() > weekAgo; }).length;

    var html = '<div class="stat-grid">';
    html += stat('Megjelent cikk', published, drafts + ' piszkozat');
    html += stat('Összes bejegyzés', posts.length, 'a blogban');
    html += stat('Az elmúlt 7 napban', weekPosts, 'szerkesztett bejegyzés');
    html += stat('Megkeresések', '→', 'az ERP-ben');
    html += '</div>';

    html += '<div class="dash-cols">';
    html += '<div class="panel"><div class="panel__head"><h3>Megkeresések</h3><a href="/erp/leads">Megnyitás →</a></div><div class="panel__body">';
    html += '<div style="padding:28px 22px;color:var(--color-text-secondary);font-size:14px;line-height:1.6;">' +
      'A weboldalról érkező briefek mostantól közvetlenül az ERP-be kerülnek, a felkutatott leadek mellé — ' +
      'pontozva, a pipeline-ban követve.<br><a href="/erp/leads" style="color:var(--color-accent);display:inline-block;margin-top:12px;">Megnyitás az ERP-ben →</a></div>';
    html += '</div></div>';
