...
(() => {
  ...
  if (brief) {
    ...
    /* Honeypot. Appended last so the positional input[type="text"] lookups
       below (name = [0], company = [1]) are unaffected. Hidden from people and
       from screen readers; a bot that fills every field gives itself away and
       /api/leads/brief silently drops the submission. */
    ...
    steps[steps.length - 1].appendChild(hp);
    ...
    if (submitBtn) {
      submitBtn.addEventListener('click', (e) => {
        ...
        /* Briefs go straight into the ERP as leads (/erp/leads), so inbound
           and scraped prospects sit in one pipeline and get scored, chased and
           speed-to-lead tracked the same way. Same-origin, so no key is needed
           here — the route checks Origin and rate-limits instead. On failure we
           still fall back to mailto so a brief is never silently lost. */
        fetch('/api/leads/brief', {
          method: 'POST',
