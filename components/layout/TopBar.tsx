...
export function TopBar({ initials = "RB" }: { initials?: string }) {
  ...
  // Paths are /erp/<section>/<id> — drop the /erp mount prefix before labelling.
  const all = path.split("/").filter(Boolean);
  const segs = all[0] === "erp" ? all.slice(1) : all;
      ...
      <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-border bg-background/80 backdrop-blur-md px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" className="gap-1.5" onClick={() => setOpen(true)}>
            ...
            New lead
            <kbd className="ml-1 inline-flex h-4 items-center rounded bg-black/15 px-1 text-[10px] font-medium">
              K
            </kbd>
          </Button>
