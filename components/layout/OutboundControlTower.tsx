...
export function OutboundControlTower({ stats }: { stats: OutboundStats }) {
  ...
  if (stats.drafts_pending > 0)
    nextActions.push({ label: `Hagyj jóvá ${stats.drafts_pending} piszkozatot`, href: "/erp/outreach" });
  if (stats.drafts_approved > 0)
    nextActions.push({ label: `Küldd el a ${stats.drafts_approved} jóváhagyottat`, href: "/erp/outreach" });
  if (stats.by_track.needs_site + stats.by_track.upgrade > 0 && stats.drafts_pending === 0)
    nextActions.push({ label: "Generálj piszkozatokat a top leadekhez", href: "/erp/outreach" });
  if (nextActions.length === 0)
    nextActions.push({ label: "Indíts egy batch vadászatot", href: "/erp/prospecting" });
