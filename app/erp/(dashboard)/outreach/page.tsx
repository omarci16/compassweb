...
export default async function OutreachPage() {
  ...
  const recipientFor = (e: typeof log[number]) => {
    if (e.project_id) {
      ...
      return { name: p?.client_name ?? e.to_address, href: p ? `/erp/projects/${p.id}` : null };
    }
    if (e.deal_id) {
      ...
      return {
        ...
        href: d ? `/erp/pipeline/${d.id}` : null,
      };
    }
    if (e.lead_id) {
      const l = leads.find((x) => x.id === e.lead_id);
      return { name: l?.company_name ?? e.to_address, href: l ? `/erp/leads/${l.id}` : null };
    }
